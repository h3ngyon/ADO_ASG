...
CREATE OR REPLACE TABLE INVENTORY_PRODUCT_PROFILE AS
SELECT
...
CREATE OR REPLACE TABLE SUPPLYCHAIN_ORDER_LINE AS
WITH internet AS (
  SELECT
...
CREATE OR REPLACE TABLE SUPPLYCHAIN_SHIPPING_MONTHLY AS
SELECT
...
FROM ASG_TRANSFORMATIONS.SUPPLYCHAIN_ORDER_LINE o
LEFT JOIN ASG_CLEAN.DIMSALESTERRITORY_CLEAN t
  ON o.SALES_TERRITORY_KEY = t.SALESTERRITORYKEY
LEFT JOIN ASG_TRANSFORMATIONS.INVENTORY_PRODUCT_PROFILE p
...
CREATE OR REPLACE TABLE INVENTORY_DEMAND_MONTHLY AS
SELECT
  DATE_TRUNC('month', o.ORDER_DATE) AS ORDER_MONTH,
  o.SALES_CHANNEL,
  o.PRODUCT_KEY,
...
CREATE OR REPLACE TABLE INVENTORY_REORDER_RISK AS
WITH demand AS (
  SELECT
    PRODUCT_KEY,
  ...
  FROM ASG_TRANSFORMATIONS.SUPPLYCHAIN_ORDER_LINE
...
FROM ASG_TRANSFORMATIONS.INVENTORY_PRODUCT_PROFILE p
LEFT JOIN calc c
