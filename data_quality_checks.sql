...
BEGIN
  ...
  IF (EXISTS (
    ...
    LET cnt_dimdate NUMBER := (SELECT COUNT(*) FROM ASG_CLEAN.DIMDATE_CLEAN);
    ...
    LET null_dimdate NUMBER := (SELECT COUNT(*) FROM ASG_CLEAN.DIMDATE_CLEAN WHERE DATEKEY IS NULL);
  ...
  IF (EXISTS (
    ...
    LET cnt_dimcust NUMBER := (SELECT COUNT(*) FROM ASG_CLEAN.DIMCUSTOMER_CLEAN);
    ...
    LET null_dimcust NUMBER := (SELECT COUNT(*) FROM ASG_CLEAN.DIMCUSTOMER_CLEAN WHERE CUSTOMERKEY IS NULL);
  ...
  IF (EXISTS (
    ...
    LET cnt_dimprod NUMBER := (SELECT COUNT(*) FROM ASG_CLEAN.DIMPRODUCT_CLEAN);
    ...
    LET dup_dimprod NUMBER := (
      ...
      FROM (
        SELECT PRODUCTKEY
        FROM ASG_CLEAN.DIMPRODUCT_CLEAN
        GROUP BY PRODUCTKEY
        HAVING COUNT(*) > 1
      )
    );
    ...
    LET null_dimprod NUMBER := (SELECT COUNT(*) FROM ASG_CLEAN.DIMPRODUCT_CLEAN WHERE PRODUCTKEY IS NULL);
    SELECT IFF(:null_dimprod = 0, 1, 1/0) AS check_dimproduct_productkey_not_null;
  END IF;

  -------------------------------------------------------------------
  -- 4) FACTINTERNETSALES_CLEAN checks
  -------------------------------------------------------------------
  IF (EXISTS (
      SELECT 1
      FROM INFORMATION_SCHEMA.TABLES
    ...
    LET cnt_fact NUMBER := (SELECT COUNT(*) FROM ASG_CLEAN.FACTINTERNETSALES_CLEAN);
    ...
    IF (EXISTS (
      ...
      LET bad_fk_cust NUMBER := (
        ...
        LEFT JOIN ASG_CLEAN.DIMCUSTOMER_CLEAN d
    ...
    IF (EXISTS (
      ...
      LET bad_fk_prod NUMBER := (
        SELECT COUNT(*)
        FROM ASG_CLEAN.FACTINTERNETSALES_CLEAN f
        LEFT JOIN ASG_CLEAN.DIMPRODUCT_CLEAN d
    ...
    -- FK to DimDate via OrderDateKey
    IF (EXISTS (
        SELECT 1 FROM INFORMATION_SCHEMA.TABLES
        WHERE TABLE_SCHEMA='ASG_CLEAN' AND TABLE_NAME='DIMDATE_CLEAN'
    )) THEN
      LET bad_fk_date NUMBER := (
        SELECT COUNT(*)
        FROM ASG_CLEAN.FACTINTERNETSALES_CLEAN f
        LEFT JOIN ASG_CLEAN.DIMDATE_CLEAN d
